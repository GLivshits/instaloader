...
class Instaloader:
    ...
    def download_pic(self, filename: str, url: str, mtime: datetime,
                     filename_suffix: Optional[str] = None, _attempt: int = 2) -> bool:
        #print('FUNCTION EXECUTED:{}'.format(inspect.currentframe().f_code.co_name))
        ...
        if os.path.isfile(filename):
            ...
            return False
        self.context.get_and_write_raw(url, filename, session = self.context._session)
    ...
    def download_title_pic(self, url: str, target: Union[str, Path], name_suffix: str, owner_profile: Optional[Profile],
        ...
        filename = self.format_filename_within_target_path(target, owner_profile, 'profile', 'pic', 'jpg')
    ...
    def download_post(self, post: Post, target: Union[str, Path], filepath: Optional[str] = None,
        ...
        dirname = os.path.join(dirname, 'posts')
        ...
        if post.typename == 'GraphSidecar':
            if (download_pics or self.download_videos):
                for edge_number, sidecar_node in enumerate(post.get_sidecar_nodes(), start=1):
                    if download_pics and (not sidecar_node.is_video or self.download_video_thumbnails):
        ...
        with resumable_iteration(
            ...
            for number, post in enumerate(posts, start=start_index + 1):
                ...
                with self.context.error_catcher("Download {} of {}".format(post, target)):
                    ...
                    # person_flag = '1 person' in caption
                    # text_flag = 'text' in caption
                    # headscarf_flag = 'headscarf' in caption
                    # print('1 person: {}. Text: {}. Headscarf: {}.'.format(person_flag, text_flag, headscarf_flag))
                    # total_flag = person_flag and (not text_flag) and headscarf_flag
                    total_flag = True
                    ...
                    if total_flag:
                        ...
                        while True:
                            ...
                            #     print('Login error detected. Changing session.')
                            #     self.context.anonymous_copy()
                            #     continue
    ...
    def username_loop(self,
                            posts: Iterator[Post],
                            target: Union[str, Path],
                            max_count: Optional[int] = None,
                            total_count: Optional[int] = None,
                            get_profiles: Optional[bool] = False,
                            owner_profile: Optional[Profile] = None,
                            download_profiles_pics: Optional[bool] = False,
                            download_profiles_meta: Optional[bool] = False) -> None:
        #print('FUNCTION EXECUTED:{}'.format(inspect.currentframe().f_code.co_name))
        ...
        with resumable_iteration(
            ...
            for number, post in enumerate(posts, start=start_index + 1):
                ...
                if max_count is not None and num_good > max_count:
                    break
                if displayed_count is not None:
                ...
                with self.context.error_catcher("Download {} of {}".format(post, target)):
                    ...
                    total_flag = person_flag and (not text_flag) and headscarf_flag
                    if total_flag:
                        profile = Profile.from_id(context = self.context, profile_id = post.owner_id)
                        print('Going to user: {}.'.format(profile.username))
                        is_business_account_flag = profile.is_business_account
                        is_joined_recently_flag = profile.is_joined_recently
                        external_url_flag = (not profile.external_url == 'null')
                        if (not is_business_account_flag) and (not is_joined_recently_flag) and (external_url_flag):
                            json_filename = '{0}/{1}/{1}-{2}'.format(target, profile.username, profile.userid)
                            print('Match!')
                            self.save_metadata_json(json_filename, profile)
                            num_good += 1
                            if get_profiles:
                                filepath = os.path.join(target, profile.username)
                                self.download_profiles(profiles = {profile},
                                                    profile_pic = False,
                                                    posts = True,
                                                    filepath = filepath,
                                                    download_pics=download_profiles_pics,
                                                    download_metadata=download_profiles_meta)
                            else:
                                filepath = None

    def get_profiles_hashtag(self, hashtag: Union[Hashtag, str],
                         max_count: Optional[int] = None,
                         post_filter: Optional[Callable[[Post], bool]] = None,
                         fast_update: bool = False,
                         ...
                         posts: bool = True,
                         get_profiles = False,
                         download_profiles_pics = False,
                         download_profiles_meta = False) -> None:
        #print('FUNCTION EXECUTED:{}'.format(inspect.currentframe().f_code.co_name))
        """Download pictures of one hashtag.
        ...
        if posts:
            self.context.log("Retrieving pictures with hashtag #{}...".format(hashtag.name))
            self.username_loop(hashtag.get_all_posts(), target, max_count=max_count, get_profiles = get_profiles,
                               download_profiles_pics = download_profiles_pics,
                               download_profiles_meta = download_profiles_meta)
        if self.save_metadata:
            json_filename = '{0}/{1}'.format(self.dirname_pattern.format(profile=target,
                                                                         target=target),
                                             target)
            ...
            with error_handler(profile.username):  # type: ignore # (ignore type for Python 3.5 support)
                ...
                if tagged or igtv or highlights or posts:
                    if (not self.context.is_logged_in and
                            profile.is_private):
                        raise ProfileIsPrivate("--login=USERNAME required.")
