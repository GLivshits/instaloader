...
class InstaloaderContext:
    ...
    def error_catcher(self, extra_info: Optional[str] = None):
        ...
        try:
            ...
            self.loginexceptioncount += 1
            print('Got this exception {} times'.format(self.loginexceptioncount))
            self.do_sleep()
            if self.loginexceptioncount % 3 == 0:
                if self.use_proxy:
                    self.proxyrotator.rotate_ip()
                    self.proxy_dict = self.proxyrotator.get_proxy_url_format()
                    self.user_agent = self.useragentrotator.random
                    self._session = self.get_anonymous_session()
                else:
                    self.do_sleep()
            if self.loginexceptioncount % 10 == 0:
                self.do_big_sleep()
        except TooManyRequestsException:
            print('Too many requests. Doing big sleep')
            self.do_big_sleep()
    ...
    def login(self, user, passwd):
        ...
        if self.request_batch <= 0:
            if self.use_proxy:
                ...
                self.proxy_dict = self.proxyrotator.get_proxy_url_format()
                self.user_agent = self.useragentrotator.random
                self._session.proxies.update(self.proxy_dict)
            self.request_batch = np.random.randint(10, 25)*6
            print('Current request batch limit: {}'.format(self.request_batch))
    ...
    def do_sleep(self):
        ...
        if self.sleep:
            secs = min(2.5 + random.expovariate(0.6), 6 + random.expovariate(1.2))/4
            time.sleep(secs)

    def do_big_sleep(self):
        #print('FUNCTION EXECUTED:{}'.format(inspect.currentframe().f_code.co_name))
        if self.sleep:
            secs = 120 + min(120*abs(np.random.randn()), 600 + random.expovariate(0.5))/2
            print('Making big sleep for {} secs.'.format(round(secs, 1)))
            time.sleep(secs)

    # @error_catcher
    def get_json(self, path: str, params: Dict[str, Any], host: str = 'www.instagram.com',
                 session: Optional[requests.Session] = None, _attempt=0) -> Dict[str, Any]:
        #print('FUNCTION EXECUTED:{}'.format(inspect.currentframe().f_code.co_name))
        ...
        try:
            ...
            if isinstance(self.proxy_dict, dict):
                sess.proxies.update(self.proxy_dict)
            if self.request_batch <= 0:
                if self.use_proxy:
                    print('Rotating IP...')
                    self.proxyrotator.rotate_ip()
            ...
            return resp_json
        except (ConnectionException, json.decoder.JSONDecodeError, requests.exceptions.RequestException, TooManyRequestsException, LoginRequiredException) as err:
            ...
            if 'Caused by ProxyError' in error_string:
                self.do_sleep()
            elif 'login' in error_string or 'Login' in error_string:
                self.do_big_sleep()
                if self.use_proxy:
                    print('Some get_json error. Rotating IP...')
                    self.proxyrotator.rotate_ip()
                    self.proxy_dict = self.proxyrotator.get_proxy_url_format()
                    self.user_agent = self.useragentrotator.random
                    self._session.proxies.update(self.proxy_dict)
    ...
    def get_raw(self, url: str, _attempt=1, session: Optional[requests.Session] = None) -> requests.Response:
        #print('FUNCTION EXECUTED:{}'.format(inspect.currentframe().f_code.co_name))
    ...
    def get_and_write_raw(self, url: str, filename: str, session: Optional[requests.Session] = None) -> None:
        #print('FUNCTION EXECUTED:{}'.format(inspect.currentframe().f_code.co_name))
        ...
        self.write_raw(self.get_raw(url, session = session), filename)
