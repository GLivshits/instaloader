...
class Profile:
    ...
    @classmethod
    def from_id(cls, context: InstaloaderContext, profile_id: Union[int, str]):
        #print('FUNCTION EXECUTED:{}'.format(inspect.currentframe().f_code.co_name))
        ...
        if data:
            ...
            profile = cls(context, {'username': data['reel']['owner']['username'].lower()})
        else:
    ...
    def is_joined_recently(self) -> bool:
        """.. versionadded:: 4.4"""
        return self._metadata('is_joined_recently')
...
def save_json(data, dst='./'):
    ...
    if data:
        ...
        with open(dst, 'wb') as f:
            ...
            json.dump(output_list, codecs.getwriter('utf-8')(f), indent=4, sort_keys=False, ensure_ascii=False)

def merge_json(data, dst='./'):
    if not os.path.exists(dst):
        save_json(data, dst)
    if data:
        merged = data
        with open(dst, 'rb') as f:
            key = list(merged.keys())[1]
            file_data = json.load(codecs.getreader('utf-8')(f))
            # remove_duplicate_data(file_data[key])
            if key in file_data:
                merged[key] += file_data[key]
        save_json(merged, dst)
...
def save_structure_to_file(structure: JsonExportable, filename: str) -> None:
    ...
    if structure.__class__.__name__ in ['FrozenNodeIterator', 'Profile']:
        json_structure = {'instaloader': {'version': __version__, 'node_type': structure.__class__.__name__},
